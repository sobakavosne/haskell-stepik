...
twoDigits2Int x y
  | isDigit x && isDigit y = read $ x : [y] :: Int
  | otherwise = 100
...
fibonacci :: Int -> Int
fibonacci n
  | isNegative = -(fibonacci' $ abs n)
  | otherwise = fibonacci' $ abs n
  where
    isNegative = n < 1 && even n
    fibonacci' 0 = 0
    fibonacci' 1 = 1
    fibonacci' x = fibs !! x
      where
...
sum'n'count x
  ...
  where
    sum'n'count' y = (sumDigits y, countDigits y)
      where
        countDigits 0 = 0
...
definiteIntegral a b g n
  ...
  | otherwise = δ * sum [(g x + g (x + δ)) / 2 | x <- [a,a + δ .. b]]
  where
    δ = (b - a) / fromIntegral n
